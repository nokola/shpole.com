<script lang="ts">
    import { type Marker, type Segment, PX_PER_SEC, TYPE_CONFIG, fmt, fmtShort } from "$lib/markers";
    ...
    let initialPxPerSec = 0;
    let zoomTimeout: any;

    const totalPx = $derived(duration * pxPerSec);
    const stripOffset = $derived(containerW / 2 - currentTime * pxPerSec);

    const overlayMarkers = $derived(
        markers.filter((m) => m.type !== "move" && m.type !== "hide" && m.type !== "pause"),
    );
    ...
    function onpointermove(e: PointerEvent) {
        ...
        if (activePointers.size === 1 && dragging) {
            ...
            if (initialPinchDist > 0) {
                ...
                // Limit zoom between 10 and 1000 px/sec
                pxPerSec = Math.max(1, Math.min(1000, initialPxPerSec * ratio));

                zooming = true;
                clearTimeout(zoomTimeout);
                zoomTimeout = setTimeout(() => (zooming = false), 150);
            }
        }
    }
    ...
    function onwheel(e: WheelEvent) {
        // Only prevent default if we're actually zooming or at limits to avoid interfering with page scroll
        // but typically in such components we want full control over wheel.
        e.preventDefault();
        const factor = Math.pow(1.1, -e.deltaY / 100);
        pxPerSec = Math.max(10, Math.min(1000, pxPerSec * factor));
...
>
    ...
    <div
        class="relative"
        style="width: {totalPx}px; transform: translateX({stripOffset}px); transition: {dragging || zooming
            ? 'none'
        ...
        <div class="relative h-[30px] rounded-[5px]" style="background: rgba(255,255,255,0.04);">
            ...
            {#each pauseMarkers as m (m.id)}
                <div
                    ...
                    style="
						left: {m.time * pxPerSec - 1}px;
						background: repeating-linear-gradient(180deg, {TYPE_CONFIG.pause.color} 0px, {TYPE_CONFIG.pause
                        .color} 3px, transparent 3px, transparent 6px);
					"
                ></div>
            {/each}
        ...
        <div class="relative h-[18px] mt-[3px]">
            {#each ticks as t (t)}
                ...
                <div
                    ...
                    style="left: {t * pxPerSec}px; transform: translateX(-50%);"
                >
                    <div
                        style="
							width: 1px;
